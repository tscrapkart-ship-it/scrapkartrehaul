  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-base text-white/40">All seller company profiles on the platform</p>
      </div>
      ...
      {companies.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-xl border border-[#262626] bg-card py-16 text-center">
          ...
          <p className="text-base text-white/40">No companies yet</p>
        </div>
      ) : (
          ...
          {companies.map((company: any) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-accent/10 text-base font-bold text-brand-accent">
                  {company.name?.slice(0, 2).toUpperCase() ?? "??"}
                </div>
                <div className="min-w-0">
                  <p className="truncate font-semibold text-white">{company.name}</p>
                  <p className="text-sm text-white/40">{company.industry_type ?? "—"}</p>
                </div>
              </div>
      ...
      <p className="text-sm text-white/20">{companies.length} compan{companies.length !== 1 ? "ies" : "y"} shown</p>
    </div>
  );
}
