export default function AdminLoading() {
  return (
    <div className="space-y-6">
      <div>
        <div className="h-8 w-44 rounded-md bg-white/[0.06] animate-pulse" />
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="rounded-xl border border-[#262626] bg-card p-5 animate-pulse">
            ...
            <div className="h-8 w-14 rounded bg-white/[0.06]" />
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-[#262626] bg-card p-5 space-y-4 animate-pulse">
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex justify-between items-center py-2 border-t border-white/[0.04]">
