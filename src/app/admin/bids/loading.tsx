export default function AdminBidsLoading() {
  return (
    <div className="space-y-6">
      <div>
        <div className="h-8 w-24 rounded-md bg-white/[0.06] animate-pulse" />
      ...
      <div className="rounded-xl border border-[#262626] bg-card overflow-hidden">
        <div className="border-b border-[#262626] px-5 py-3 flex gap-8">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-3 w-20 rounded bg-white/[0.06] animate-pulse" />
          ))}
        </div>
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="flex gap-8 px-5 py-4 border-b border-white/[0.04]">
