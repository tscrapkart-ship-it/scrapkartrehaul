...
async function getUsers(role?: string) {
  ...
  if (role && role !== "all") {
    query = query.eq("role", role as "recycler" | "waste_producer");
  }
...
export default async function AdminUsersPage({
  ...
  const tabs = [
    ...
    { value: "recycler", label: "Buyers" },
    { value: "waste_producer", label: "Sellers" },
  ];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-white/40">
          All registered buyers and sellers on the platform
        </p>
      </div>
